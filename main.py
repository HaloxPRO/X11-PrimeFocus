...
# creates GUI
root = Tk()
root.title("Untitled - PrimeFocus")
#root.protocol("WM_DELETE_WINDOW", lambda: __quit(1))
status = Label(root, text="", bd=1, relief=SUNKEN, anchor=W)                                # all this is for status bar
status.pack(side=BOTTOM, fill=X)
root.geometry('{}x{}'.format(900, 500))

fontdict = {"family": "Calibri", "size": 14, "style": "normal"}

e = ScrolledText(root, bg='black', fg='white', insertbackground='white', selectbackground='blue', font=(fontdict["family"], fontdict["size"], fontdict["style"]), undo=True)
e.pack(fill="both", expand=True)


# Shortcuts
root.bind("<Escape>", lambda x: root.attributes('-fullscreen', False))         # only if u use F11 will F5 work
...
# status bar
def wordcount():
    status.config(text="Character count: " + str(len(e.get(1.0, "end-1c"))) + "      Word count: " + str(len(e.get(1.0, "end-1c").split())) + "      Line count: " + str(len(e.get(1.0, "end-1c").split("\n"))))   # returns line count)
    root.after(200, wordcount)      # for refreshing or something


# create new file
def new(self):
    if root.title() == "Untitled - PrimeFocus":
        # There is content?
        if len(e.get('0.0', END)) > 1:          # non empty
            if messagebox.askyesno("Save?", "Do you wish to save?"):
                root.filename = filedialog.asksaveasfilename(parent=root, title="Save as",
    ...
    # titled
        ...
        if len(e.get('0.0', END)) != (len(handle.read())+1):  # if it has been modified
            handle.close()
            if messagebox.askyesno("Save?", "Do you wish to save?"):
    ...
    if len(e.get('0.0', END)) > 1:
        ...
        if root.title() == "Untitled - PrimeFocus":
            if messagebox.askyesno("Save?", "Do you wish to save?"):
                ...
                # then open new file
                root.filename = filedialog.askopenfilename(parent=root, title='Open File',
                                                               filetypes=(("Text file", "*.txt"), ("All files", "*.*")))
                ...
                if root.filename:
                    try:
                        with open(root.filename, "r", encoding="utf8") as w:
                            e.delete(0.0, END)
                            root.title(os.path.basename(root.filename) + " - PrimeFocus")
                            e.insert(END, w.read())
                    except:
                ...
                # open window pops up
                root.filename = filedialog.askopenfilename(parent=root, title='Open File', filetypes=(("Text file", "*.txt"), ("All files", "*.*")))
        ...
        # ------------------ TITLED --------------------
            ...
            if len(e.get('0.0', END)) != (len(handle.read()) + 1):
                ...
                if messagebox.askyesno("Save?", "Do you wish to save?"):
                    ...
                    # then open file
                    x = filedialog.askopenfilename(parent=root, title='Open File',
                                                   filetypes=(("Text file", "*.txt"), ("All files", "*.*")))
                    ...
                    if x:
                        try:
                            with open(x, "r", encoding="utf8") as w:
                                e.delete(0.0, END)
                                root.title(os.path.basename(x) + " - PrimeFocus")
                                e.insert(END, w.read())
                        except:
                            messagebox.showerror("Open file", "Could not open that file.")
                            return
...
def save(self):
    if root.title() == "Untitled - PrimeFocus":
        root.filename = filedialog.asksaveasfilename(parent=root, title="Save as", filetypes=(("Text file", "*.txt"), ("All files", "*.*")), defaultextension='.txt')
...
def checksum(algorithm, data):
    ...
    # text editor has name
        ...
        try:
            filepath = root.filename                                 # full path of file - root.filename() does not work
            handle1 = open("C:/Users/DELL/Desktop/checksum1.txt", 'r+')              # open file in read and append mode
            handle2 = open("C:/Users/DELL/Desktop/checksum2.txt", 'a')               # open file in append mode
...
def dataintegrity(fp):
        handle1 = open("C:/Users/DELL/Desktop/checksum1.txt", 'r')
        handle2 = open("C:/Users/DELL/Desktop/checksum2.txt", 'r')
        ...
        for i in lines2:                                                    # scan checksum2
            ...
            for j in lines1:
                ...
                if i == j:
                    messagebox.showinfo("Data Integrity", " File " + os.path.basename(fp) + " is not modified or corrupted.")

                    # remove entries of that filename
                    handle1.close()
                    handle1 = open("C:/Users/DELL/Desktop/checksum1.txt", 'w')
                    for line in lines1:
                        if fp not in line:
                            handle1.write(line)
                    handle1.close()
                    ...
                    handle2.close()
                    handle2 = open("C:/Users/DELL/Desktop/checksum2.txt", 'w').close()

                elif path1 == path2 and alg1 == alg2 and hashvalue1 != hashvalue2:
                    messagebox.showinfo("Data Integrity", "File " + os.path.basename(fp) + " has been modified or corrupted.")
...
def goto(self):
    ...
    Button(top, text="Cancel", command=lambda: top.destroy()).pack(side="right", fill="both", expand=True)
...
def find(self):
    ...
    # ---
    if occurences > 0:
        messagebox.showinfo("Results", findstring + " has " + str(occurences) + " occurences.")
    else:
...
def replace(self):
    ...
    if occurences > 0:
        ...
        if (messagebox.askyesno(title="Replace", message="Do you want to replace the word " + findstring + " with something else?") == True):
            ...
            while pos_start:
                ...
                print(pos_start, pos_end)  # 1.6 1.6+5c :for first occurence of searched word
...
def set_font_size(size):
    e.config(font=(fontdict["family"], size, fontdict["style"]))                             # other two are defaults
    fontdict["size"] = size


# Font type
def set_font_family(family):
    e.config(font=(family, fontdict["size"], fontdict["style"]))                             # other two are defaults
    fontdict["family"] = family


# Font style
def set_font_style(style):
    e.config(font=(fontdict["family"], fontdict["size"], style))                             # other two are defaults
...
# Help function
def help():
        messagebox.showinfo("Features available", "1. Saving, Opening text files\n2. Get checksum for file\n3. Goto line number"
...
# About function
def about():
        messagebox.showinfo("About", "PyEdit\nA Python Text Editor!!\nBetter Clone of Notepad\nVersion 5.0\nDeveloped by Joel and Ajay\nCopyright© 2018. All rights reserved.")
...
checksum_menu.add_command(label="Simple Hash", command=lambda: checksum("simple hash", data=(e.get(1.0, END))))
...
cryptographichash_menu.add_command(label="MD5", command=lambda: checksum("md5", data=(e.get(1.0, END)).encode("utf-8")))
cryptographichash_menu.add_command(label="SHA1", command=lambda: checksum("sha1", data=(e.get(1.0, END)).encode("utf-8")))
cryptographichash_menu.add_command(label="SHA224", command=lambda: checksum("sha224", data=(e.get(1.0, END)).encode("utf-8")))
cryptographichash_menu.add_command(label="SHA256", command=lambda: checksum("sha256", data=(e.get(1.0, END)).encode("utf-8")))
cryptographichash_menu.add_command(label="SHA384", command=lambda: checksum("sha384", data=(e.get(1.0, END)).encode("utf-8")))
cryptographichash_menu.add_command(label="SHA512", command=lambda: checksum("sha512", data=(e.get(1.0, END)).encode("utf-8")))
...
editmenu.add_command(label="Delete all", command=lambda: delete_all(1), accelerator="Ctrl+D")
editmenu.add_command(label="Insert time", command=lambda: e.insert(INSERT, time.ctime()), accelerator="F5")
editmenu.add_command(label="Insert colour", command=lambda: e.insert(INSERT, colorchooser.askcolor()), accelerator="F4")
...
sizemenu.add_command(label="Increase size", command=lambda: set_font_size(fontdict["size"] + 5), accelerator="Ctrl+Shift++")
sizemenu.add_command(label="Decrease size", command=lambda: set_font_size(fontdict["size"] - 5), accelerator="Ctrl+Shift+-")
...
fontstyle.add_command(label="Bold Italic", command=lambda: set_font_style("bold italic"))
...
toggle1 = BooleanVar()
viewmenu.add_checkbutton(label="Fullscreen", command=lambda: root.attributes("-fullscreen", not root.attributes('-fullscreen')), variable=toggle1)
